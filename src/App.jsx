...
import { STORAGE_KEY, TABS, DEFAULT_API_ENDPOINT, DEFAULT_MODEL, DEFAULT_PROFILE, DEFAULT_COACH_CONFIG } from "./constants";
...
export default function App() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        logs, races, chatMessages, apiKey, apiEndpoint, apiModel, itraPI, profile, coachConfig,
      }));
    } catch {}
  }, [logs, races, chatMessages, apiKey, apiEndpoint, apiModel, itraPI, profile, coachConfig]);
  ...
  // First-time setup: if required profile fields are missing on first load, force the setup wizard
  useEffect(() => {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "1.25rem 1.5rem", fontFamily: "var(--font-sans)", color: "#111" }}>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, gap: 10 }}>
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 500, margin: 0, color: "#111" }}>Wilf's Training Studio</h2>
          <p style={{ fontSize: 14, color: "#888", margin: "4px 0 0" }}>Personal training hub · Built for race goals</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #e8e8e8", marginBottom: 22, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)} style={{
            background: "none", border: "none", borderBottom: tab === i ? "2px solid #111" : "2px solid transparent",
            padding: "12px 22px", fontSize: 15, fontWeight: tab === i ? 500 : 400,
            color: tab === i ? "#111" : "#888", cursor: "pointer", marginBottom: -1, whiteSpace: "nowrap", flexShrink: 0,
          }}>{t}</button>
        ))}
      </div>
