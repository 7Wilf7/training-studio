  ...
  return (
    <div data-period-control style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap", position: "relative" }}>
      <button onClick={() => { setPeriod({ type: "all" }); setPeriodDropdown(null); }}
        style={s.chip(period.type === "all")}>All Time</button>
      ...
      <div style={{ position: "relative" }}>
        <button onClick={(e) => {
          ...
          {period.type === "week" ? getPeriodLabel(period) : "This Week"} <span style={{ marginLeft: 4, fontSize: 9 }}>▾</span>
        </button>
        {periodDropdown === "week" && (
          <div style={{ position: "absolute", top: "100%", left: 0, marginTop: 4, background: "#fff", border: "1px solid #ddd", borderRadius: 8, padding: 6, zIndex: 100, minWidth: 140, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
            {[0, -1, -2, -3, -4].map(off => (
              <button key={off} onClick={() => { setPeriod({ type: "week", offset: off }); setPeriodDropdown(null); }}
                ...
                {off === 0 ? "This Week" : off === -1 ? "Last Week" : `${Math.abs(off)} weeks ago`}
              </button>
            ))}
          </div>
        )}
      ...
      <div style={{ position: "relative" }}>
        <button onClick={(e) => {
          ...
          {period.type === "month" ? getPeriodLabel(period) : "This Month"} <span style={{ marginLeft: 4, fontSize: 9 }}>▾</span>
        </button>
            ...
            {pastMonths(24).map((m, i) => {
              ...
              return (
                <button key={i} onClick={() => { setPeriod(isCurrent ? { type: "month" } : { type: "month", year: m.year, month: m.month }); setPeriodDropdown(null); }}
                  ...
                  {isCurrent ? "This Month" : getPeriodLabel({ type: "month", year: m.year, month: m.month })}
                </button>
              );
            })}
          </div>
      ...
      <div style={{ position: "relative" }}>
        <button onClick={(e) => {
          ...
          {period.type === "year" ? getPeriodLabel(period) : "This Year"} <span style={{ marginLeft: 4, fontSize: 9 }}>▾</span>
        </button>
        {periodDropdown === "year" && (
