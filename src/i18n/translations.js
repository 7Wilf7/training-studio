...
const dict = {
  ...
  "api.desc":                 { en: "Used by AI Coach chat and race lookup. Pick a preset below to fill URL+model in one click, then paste your API key. Stored only in this browser's localStorage.", zh: "用于 AI 教练对话和比赛信息查询。点下面的预设可一键填入 URL 和模型，再粘贴你的 API Key。只保存在本浏览器的 localStorage 中。" },
  ...
  "races.lookup_web":         { en: "Look up online (web search)",          zh: "联网搜索查询" },
  "races.lookup_web_hint":    { en: "🔍 Live web search — dates and categories filled in automatically when found. Requires an endpoint that supports the web_search tool.", zh: "🔍 联网搜索 —— 找到后自动填入日期和组别。需要使用支持 web_search 工具的接口。" },
  "races.lookup_no_key":      { en: "No API key set. Click the 🔑 API button (top-right) first.", zh: "未设置 API Key。请先点右上角的 🔑 API 按钮。" },
  "races.lookup_searching_web":{ en: "Searching the web…",  zh: "正在联网搜索……" },
